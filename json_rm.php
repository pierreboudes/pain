...
function json_rm_php($id, $readtype) {
    ...
    } else if ($readtype == "etapeformation") {
        $code_etape = getclean("id");
	    $id_par = getnumeric("id_parent");
	supprimer_etapeformation($code_etape, $id_par);
    } else if ($readtype == "etapecours") {
	$id_par = getnumeric("id_parent");
	supprimer_etapecours($id, $id_par);
    } else {
